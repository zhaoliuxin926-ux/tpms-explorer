...
Agent 链路分五层，每层只信任下一层的结构化输出，绝不跳层：
...
```bash
node tpms/agent/schema_check.mjs   # 契约对拍：schema 边界 vs CLI 校验，【SCHEMA_N】断言
...
拦截器对了，不代表模型好用。回归集从 34 条中英设计指令起步，覆盖参数类意图 100%（nl-agent 语义覆盖表对拍）。四档真实模型的终态：

| 模型 | 通过 | 备注 |
|---|---|---|
| glm-4-flash（经济档） | 33/37 | 弱项全部被拦截器安全兜住 |
| glm-4.6 | 35/37 | |
| glm-5.3（满血） | 36/37 | 失败项轮换 |
| glm-5.3-flash | **37/37** | 一次全绿 |

关键观察是"**失败项轮换**"：满血模型这次挂的指令，直跑一次就过——方差来自推理随机性，不是管线缺陷。所以验收语义是三层而不是一层：**拦截器保证"错的不执行"（确定性），回归保证"好的能通过"（统计性），对抗指令保证"坏的进不来"（四模型零透传）**。把统计性验收当成确定性保证来宣传，是 Agent 评测最常见的造假姿势。
...
分工刻意不对称：**梯内修复由确定性梯完成**（比如孔隙率偏差超阈时的分辨率提升，根本没有 LLM 什么事）；LLM 只接管"梯外"的策略选择。验收是注入故意缺陷后 ≤5 轮自动收敛全绿，全程无人工干预；LLM 的每个修复决策同样过拦截器钳制，轮数耗尽 exit 4、参数层拒绝 exit 3、拦截器拒绝 exit 2——退出码本身是契约。

一个真实收敛案例：fcks 曲面 R120 超出验证梯 → 换族 gyroid R128 两轮收敛；gprime k=6 周期超标定域 → 降 periods 5 失败 → 自动换族 diamond 三轮收敛。**"失败后换个方向再来"恰恰是 LLM 擅长的部分**——但每个决策的执行和验收，依然是确定性代码说了算。
...
3. **每个功能宣称必须指认一条跑过的命令。** 拦截器存在 ≠ 拦截在执行路径上；校验函数通过单测 ≠ 非法输入真的被拦。这条纪律贯穿两篇文章，也是它作为方法论的自证：本文所有断言数字来自文首三条命令的当轮实测。
