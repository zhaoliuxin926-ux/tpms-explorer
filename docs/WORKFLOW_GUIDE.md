...
> **学术渲染与论文汇报**。主链路导出经 CI 门禁审计（见文末质量基准表）；能力限制与未上机项见 [HONESTY_BOUNDARIES](HONESTY_BOUNDARIES.md)。
>
...
34. [组织工程成骨长入与微流体氧气输运反应-扩散仿真](#三十四组织工程成骨长入与微流体氧气输运反应-扩散仿真--v70) 🆕 原型期 v7.0
35. [水平集拓扑导数自适应多目标优化重构](#三十五水平集拓扑导数自适应多目标优化重构--v70) 🆕 原型期 v7.0
36. [C2 曲面族扩展与可用域速查](#三十六c2-曲面族扩展与可用域速查--v70) 🆕 原型期 v7.0
...
## 三十一、隐式神经场（SIREN）潜在拓扑流形探索 🆕 原型期 v7.0
...
## 三十二、三维多轴各向异性屈服面与多孔破坏裕度分析 🆕 原型期 v7.0
...
## 三十三、声子晶体能带色散与超材料宽频振动隔离设计 🆕 原型期 v7.0
...
## 三十四、组织工程成骨长入与微流体氧气输运反应-扩散仿真 🆕 原型期 v7.0
...
## 三十五、水平集拓扑导数自适应多目标优化重构 🆕 原型期 v7.0
...
| 批次 | 曲面 | 谐波 | 备注 |
|---|---|---|---|
| 经典 8 | Gyroid / Diamond / Schwarz P / Neovius / I-WP / F-RD / Lidinoid / Split-P | ≤2× | 教学版同步 |
| C2-1 | Octo (O,C-TO) / Karcher (K) / FK-S / FK-Y / G′ | ≤2× | Schoen 立方族补缺 + Fischer-Koch |
| C2-2 | FK-C(S) | 3× | 恒等式优化（21 三角→6 三角+纯乘法） |
| C2-3 | D′ / Double P / Double D / Double G | ≤2× | MiniSurf 展示方程 |
| C2-4 | FK-C(Y) | ≤2× | 与 FK-Y 低频反号 |
| C2-5 | Complementary D | 3× | 混合角展开 12 项 |

### 36.2 可用域决策表（p0.6，k=6 默认）

完整矩阵见仓库根 [`BENCHMARKS.md`](../BENCHMARKS.md)。要点：

- **低谐波族（≤2×）**：R48/R96 基本可产；例外 frd/fks/fky/dprime/cdd R48 薄壁自触 fail-closed，R96 可产
- **gprime**：k6 R96 拒产（nm 19080），**降周期数 k=2 可产**
- **fcks（谐波 3×）**：可产域 = **R120 k6**（p0.5–0.7）∪ **R128 k2**（~9s，偏差 0.13pp）；k6 R48/R96/R128 拒产
- **dg**：可产但 p0.6 iso 触求解域下界，偏差 ~8pp 如实报告
- **拒产是正确行为**：水密门 fail-closed 代表该 (曲面, 孔隙率, 分辨率) 组合的网格表示不可靠，CLI 会给出结构化诊断
...
### v7.0 变更总览（36 门 · 1000+ 断言；历史口径，现 45 门）
...
工程版「视图与工具→工程观察」内「径向梯度构型 M(r)」卡：K/ta/tb 滑块（K=1 自动锁 tb=ta；nTop 标定表精确落格，0.001 步长）+ 「生成预览 (MT)」（R=48 一次性预览，实测孔隙率读数）+ 「导出 STL (HD)」（R=96，内置边配对水密审计门——与 CLI 水密门同标准，拒绝即不导出）。守卫与 CLI 同义：须 Schwarz P + solid_network + cube，与外部 STL 容器/渐变等值场/混合场/非欧映射互斥。预览为一次性视图（改参后常规重建即恢复）；与 CLI 逐位同源（K=1.5 实测 192,784 三角/54.7%）。
