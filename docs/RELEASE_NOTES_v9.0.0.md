...
- **SDF 主线程同步执行**：STL 容器上传时 `computeMeshSDF` 在主线程同步运行（冒烟锚点 12 三角）——大体量解剖网格（>10⁴ 三角）将阻塞 UI 数秒；Worker 化管线已立项归档 `bugs.md`，待真实临床需求触发。
  - 【封板后加固补记 2026-09-13】本条以 tag 快照（567622d）为准；**SDF Worker 化已在封板后加固提交 e098c30 落地**（动态 Worker / Transferable 零拷贝 / 竞态 id 防护），main 最新状态不再受此边界约束。
