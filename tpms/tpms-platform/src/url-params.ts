...
const VALID = {
  type: ['gyroid','diamond','schwarz','neovius','iwp','frd','lidinoid','splitp','octo','karcher','fks','fky','gprime','custom'] as TpmType[],
...
export function parseURLParams(search: string): Partial<AppState> {
  ...
  if (hdRaw && ['gyroid', 'diamond', 'schwarz', 'neovius', 'iwp', 'frd', 'lidinoid', 'splitp', 'octo', 'karcher', 'fks', 'fky', 'gprime'].includes(hdRaw)) {
    state.hierarchical = {
      enabled: true,
