...
const VALID = {
  type: ['gyroid','diamond','schwarz','neovius','iwp','frd','lidinoid','splitp','octo','karcher','fks','fky','gprime','fcks','custom'] as TpmType[],
...
export function parseURLParams(search: string): Partial<AppState> {
  ...
  if (hdRaw && ['gyroid', 'diamond', 'schwarz', 'neovius', 'iwp', 'frd', 'lidinoid', 'splitp', 'octo', 'karcher', 'fks', 'fky', 'gprime', 'fcks'].includes(hdRaw)) {
    state.hierarchical = {
      enabled: true,
