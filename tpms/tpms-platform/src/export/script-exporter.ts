 ...
 * 【v4.0 红测抓获的存量 bug】模板对插值是无条件求值——非 custom 类型（customFormula='')
 * 会在字符串构建期就编译空公式抛错，导致 8 类内置曲面的 py/m 导出全数崩坏
 * （16 门从未以非 custom 类型导出 py，故漏网）。空公式回退语法合法的占位表达式
 * （python 分支不执行，仅保语法）。
 */
function customPyExpr(s: AppState): string {
  if (!s.customFormula.trim()) return '0.0  # 非自定义模式：此分支不执行';
  return getCompiledCustomFormula(s.customFormula).toPython((ax) => `kk*${ax.toUpperCase()}`);
}

/** 自定义公式 → MATLAB 表达式（.* ./ .^ 向量化；空公式占位同上） */
 ...
 */
/** 纯构建（audit 静态断言用）：Python 重建脚本全文 */
export function buildPythonScript(state: AppState): string {
  return buildPythonScriptImpl(state);
}

function buildPythonScriptImpl(state: AppState): string {
...
manifold_kind = '${safeId(state.manifold.kind)}'
if manifold_kind != 'identity':
    ...
    if manifold_kind == 'poincare':
        ...
        sc = ${state.manifold.scale}
        ax = '${safeId(state.manifold.axis)}'
...
export function exportPythonScript(state: AppState, filename: string): void {
  downloadBlob(new Blob([buildPythonScript(state)], { type: 'text/plain' }), filename);
}

/** MATLAB 脚本纯字符串构建（2026-09-15 拆出：Node 侧可加载实测——本机 MATLAB R2025a/b 实跑闭环） */
export function buildMatlabScript(state: AppState): string {
...
export function exportMatlabScript(state: AppState, filename: string): void {
  downloadBlob(new Blob([buildMatlabScript(state)], { type: 'text/plain' }), filename);
}
