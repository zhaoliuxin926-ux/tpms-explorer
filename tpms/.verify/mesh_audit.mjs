...
const CASES = [
  ...
  // C2 第五批：cdd（谐波 3×，R96 可产 / R48 薄壁自触——与 dprime 同族，2026-09-11）
...
for (const r of results) {
  ...
  // preview 档 0.12：R28/k3（每周期 ~9.3 格）采样极限——2026-09-06 投影 k 倍步长修复后
  // 正确投影暴露粗拓扑真实缺料（-6.03%→-9.83%，旧混沌过冲偶然垫高体积），
  // 本组案例守卫角色 = 极粗档拓扑/水密/定向回归（同 gradshell 先例）
  const volTol = isHybrid ? 0.08 : (r.preview ? 0.12 : (r.tolGrad ? 0.18 : (r.tol2k ? 0.12 : 0.06)));
