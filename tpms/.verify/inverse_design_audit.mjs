 ...
 * B. 解剖预设：cortical/trabecular/heatsink 三组——参数在可行域内 + J 有限 +
 *    存在命中约束 ≤3%（多条约束中至少命中强权重约束之一，非每条 ≤3%）
 * C. 类型枚举与解完整性：8 类型全部返回、类型合法、J 升序排列
 *    （【2026-09-10 头注对齐】原「参数在可行域内」仅 B 节断言，C 节无此检查）
 * D. 确定性：同输入两次求解结果逐位一致
