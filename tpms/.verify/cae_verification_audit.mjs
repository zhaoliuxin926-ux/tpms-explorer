...
console.log('\n[A] 求解脚本语法（python ast 解析）');
{
  ...
  for (const [name, text] of scripts) {
    ...
    const res = spawnSync(process.platform === 'win32' ? 'python' : 'python3', ['-c', `import ast; ast.parse(open(r'${tmpPy.replace(/\\/g, '/')}', encoding='utf-8').read()); print('OK')`], { encoding: 'utf8' });
...
console.log(`\nRESULT: ${passCount} PASS / ${failCount} FAIL`);
  if (passCount < 25) { console.error('GUARD FAIL: 断言执行数 ' + passCount + ' < 基线 25（恒真/集体跳过防护，2026-09-04 审查纳管）'); process.exit(1); }
if (failCount > 0) {
  console.log('失败项:');
