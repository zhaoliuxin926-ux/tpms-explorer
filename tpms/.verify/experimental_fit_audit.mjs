 ...
 *
 * 断言矩阵（18 项 + 1 活性探针）：解析容错 ×8（分隔符自适应 ×3/表头+列名/乱序+重复折叠/非有限值
...
// H-2 修复（红队 D）：比较器活性探针——守卫防删除类，恒真/中和类靠此探针兜底
//（rel 被中和成 ()=>-1 时 rel(9999,E0) 返回 -1 ≤0，探针红；正常 rel 返回 7.33>0.5 绿）
rel(9999, E0) > 0.5 ? ok('比较器活性探针（负样本 rel(9999,1200)=7.33 必 >0.5）', rel(9999, E0).toFixed(2)) : bad('比较器疑似被中和（恒真守卫盲区）', String(rel(9999, E0)));
