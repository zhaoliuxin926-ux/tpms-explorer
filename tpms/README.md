...
**自然语言 → schema 逐槽位校验拦截器（越界拒绝） → 确定性 CLI 执行 → 门禁验证 → 有界修复循环**——LLM 只填意图槽位，一切数值由确定性代码生成或校验：
...
| **曲面族** | 8 类经典（教学聚焦） | **24 类**（C2 扩展含 Fischer-Koch / Double / Complementary D / Slotted P / F / Q* / W 等） |
