...
## 附录（2026-08-27）：网格管线 v2 全面重写

**问题**：导出 STL 完全不可用——开放边 4.7 万/模型、非流形边 7 万、体积偏差 ±200%，且整体反相（封闭侧为空气补集）。

**根因**：① 面提取不检查「网格边穿越」；② `max(F, bound)` 正壳使实体与外壳融合，封闭侧变成空气相；③ 圆柱角落公式正瓣压过裁剪；④ 平滑跨薄壁拉扯 + 投影缺位。

**v2 方案**（三方同源：平台 surface-nets.ts / 单文件 app.html / Python·MATLAB 导出脚本）：
- 面提取以「网格边穿越」为键（Lysenko surface-nets 不变量）——水密由构造保证
- 统一封盖规则：容器外/网格边界层无条件空气覆写；孔口环质心扇面封盖（反向配对自动定向）
- 切向平滑（剥法向分量）+ 解析 Newton 投影（solid 模式），体积保真机制化
- 倍频曲面（I-WP/F-RD/Neovius）分辨率密度加倍；梯度壳 radial/spherical 同步补偿
- 实测口径：孔隙率/材料占比/物理指标全部来自网格发散体积（替代有 13pp 量化偏差的格子 MC）
- 入口守卫：退化场/非有限公式/超池分辨率显式报错 + UI toast；零法线离散兜底；采样定理警示

**验证**：`tpms/.verify/mesh_audit.mjs` 26/26 案例（8 曲面 × 3 结构模式 × 2 容器 × hybrid × 梯度×3 方向 × preview × 非对称权重 × 5 红队回归）全过——开放边全部为 0，固相体积偏差 ≤6%（文档化容忍带内 ≤18%）。红队 53 用例攻击矩阵：静默失败通道清零。全量回归（parity 74/74、run_all 108 项、fix_check 23/23、verify 18/18、终验 9/9）全绿。详见 `tpms/agent_memory/mission_2026-08-27.md`。
