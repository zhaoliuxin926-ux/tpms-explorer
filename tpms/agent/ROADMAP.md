...
### 版本发布规则（2026-09-18 版本纪元重置定案）
- **版本纪元**：2026-08 冲刺期版本号至 v9.2.0（12 tag）为原型期快照（每日战役节奏下的里程碑记录，密度实录：三天 v2.2→v7.0、隔日 v8→v9）；历史 tag/Release/RELEASE_NOTES 保留不重写。**自 v1.0.0 起进入语义化稳定序列**。
- **bump 判定**：minor=用户可见的新能力批次（新曲面族/新构型管线/新交付格式）；patch=缺陷修复与小改进；文档、门禁扩容、独立探针进 main 不发版。**论文对齐绑定 minor 轮**（同轮完成，v18 教训）。CI 三平台绿后才打 tag @tip（既有纪律）。
...
**战役 A（S1-S11 吸收）**：平台 INP 是 \*STATIC 求解——S1-S11 的 Explicit 专属项（SmoothStep/DMASS/质量缩放）不能教条照搬，对症吸收三项：① INP 导出器补 `*OUTPUT, HISTORY` + NSET_TOP 的 RF/U（压缩曲线数据源——此前只有场输出，跑完拿不到反力曲线，对接 experimental-fit 的真缺口）；后处理口径 σ=ΣRF3/A0、ε=−U3/H0（与论文基线 S11 同口径）。② 协议 §九 仿真侧对照（Explicit 路线判据表：KE/IE<5%/应变率反推式/μ 文献带/k 披露制，指向论文协议本地路径）。③ 门禁 cae_mesh_audit +3（INP 历史输出断言，46→60 路上）。

**战役 B（M(r) 度规梯度几何）**：论文核心创新的空间映射移植为平台构型——`core/radial-grad.ts`（arctanh 径向+有理轴向双通道逆映射/阈值场 C_lin 壁厚补偿/kWall=0.2444/K=1 退化均匀 P C=0.871，公式逐行出自论文 scaffold_conformal_map.py）。接入 surface-nets（tpmFn 坐标包装与 transformByStress 同构 + 平方壳场 P²−C(r)² 与 shell 机制同构）+ CLI `mesh --radial-grad K [--ta/--tb]`（periods 语义变域内晶胞数，论文口径 12）。**四个实现迭代（每个都是实测定案）**：① cube 角区 r1>1 触发封口截断填实 → 强制 cylinder 域（论文圆柱口径）；② **|P| 绝对值场在 P=0 骨架面折痕是 surface-nets 结构性毒药**（K=1 也 nm=992；论文 MC 查表无此问题）→ 改数学等价平方场 P²−C²（nm 4008→200）；③ 封口跳变帽（C≥1.3→3）界面非流形假说证伪（连续化后 nm=232；C_lin 域内最大 1.318<3 实无封口带）→ 定案自然增长；④ 残余 nm=64-1024 全参数域存在（r>0.78 外缘+端面带=surface-nets 容器交线深水区，与 cylinder+diamond 族同源）。**定案：数学层完成验证、STL 产出 fail-closed 登记**——K=1.5 密度锚 **50.0-50.6% 命中论文设计值 50%**（跨尺度物理对拍）、数学不变量锚（A·C_rad=K/双通道极限 1/K/边缘恢复 1）全过；**MC 类提取器登记为待攻战役**（平台复现论文几何的最后一块）。门禁 F 组 +5（65 总）。壁厚体素比守卫（ta·R/periods≥2 fail-fast）。同源自拍教训：F1 双实现对拍两次踩坐标换算坑 → 换论文记载数学不变量锚更强。

**战役 C（batch5 数据指引）**：协议 §六 补历史数据复用指引（本地路径+格式转换+**红线：论文未发表原始数据不入仓库**——预公开损害原创性声明）。

**验证**：cae 65/0 + tsc 0 + 全量 44 门复验（见套件输出）。
